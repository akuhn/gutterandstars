...
% ---- ----- ----- ----- ----- ---- ---
\paragraph{Observation 7.1: When thinking aloud, developers spoke of the system's architecture in spatial terms} 
...
\paragraph{Hypothesis 7.1: Professional developers establish a spatial mental model of the system's architecture}
...
\paragraph{Implication 7.1: Developers should be able to arrange the layout according to their mental model}
...
\paragraph{Observation 7.2: Participants used Codemap to assess quantity and dispersion of search results and call graphs}
...
\paragraph{Hypothesis 7.2: Intuitive visualizations to show quantity and dispersion of search results (as well as call graphs) address an important need of developers}
...
\paragraph{Implication 7.2: Tools should put search results into a meaningful context, so developers can take both quicker and better-informed decisions}
...
\paragraph{Observation 7.3/a: When interacting with the map, participants were attracted to isolated elements, rather than exploring clusters of closely related elements}
...
\paragraph{Hypothesis 7.3/b: Participants rarely used Codemap to return to previously visited locations, instead using package explorer and ``Open Type'' to do so}
...
\paragraph{Hypothesis 7.3: Developers avoided clusters of closely related elements because they are difficult to identify and select on the map}
...
\paragraph{Implication 7.3: The map's layout should be such that all elements are easily discernable and easy to click}
...
\paragraph{Observation 7.4: Participants rarely used Codemap to return to previously visited locations, instead using package explorer and ``Open Type'' to do so}
 
\paragraph{Hypothesis 7.4:  Developers failed to establish a spatial memory of the map not due to its layout but also due to missing visual cues}
...
\paragraph{Implication 7.4: It should be possible to bookmark classes as ``top landmarks,'' both manually and automatically based on usage statistics}
...
\paragraph{Observation 7.5: Participants used Codemap as if its layout were based on package structure\,---\,even though they were aware of the underlying topic-based layout}
...
\paragraph{Hypothesis 7.5: From the developers view, the predominant mental decomposition of a system is package structure}
...
\paragraph{Implication 7.5: The map layout should be based on code structure rather than latent topics only. However, non-structural data should be used to enrich the layout}
...
In this chapter we presented an evaluation of spatial software visualization in the IDE. We embedded a prototype of the \emph{\SOCA} approach~\cite{Kuhn08a,Erni10a,Kuhn10b}, the \Codemap plug-in, in the Eclipse IDE and ran an exploratory user study which included both students and professionals. 
