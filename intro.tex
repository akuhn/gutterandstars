...
In this work, we present the following contributions that explore the support of code orientation clues by development tools. Each of these contributions has been published as one or more peer-reviewed paper at an international venue or in an international journal.  

Common to all these contributions is that tap on unconventional information found in source code, in order to provide developers with new means of following-up code orientation clues that had been out of their reach without the novel tool support. For example, in \autoref{the chapter on chronia} we present a story-relling visualization of a system's history that enables new hires to draw on episodic memories that they would not have access to otherwise.
...
%
We implemented our approach in prototype and evaluated it in a user study, and found that it is most helpful for spatially exploring search results and call hierarchies.

For more information please refer to the next section for an introduction in a nutshell, as well as \autoref{the chapter on codemap} and \autoref{the chapter on the codemap user study} for an in depth coverage. 
...
Given current tool support, following up social clues has to be done through the lexical proxy of a persons name. We present an approach to discover experts without having to know their name. Given a problem description, such as a work item or a bug report, we provide an automated means of linking to the person with the expertise on that matter. We use lexical information found in the submission to the version control system in order to model the developer's expertise. 
%
