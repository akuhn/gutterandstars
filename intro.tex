...
Despite common belief, software engineers do not spend most time writing code. An approximate 50--90\% of development time is spend on code navigation and understanding \cite{Abra04a,Lato06a,more}. This may include reading of local source code and documentation, searching the internet for tutorials and code examples, but also seeking the help of other developers. User studies have found that software engineers use at least four kinds of cognitive cues for \codenavigation: 
	lexical cues, %\cite{a,b,c,d,e}, 
	social cues, \cite{Lato06a,more}, 
	episodic cues %\cite{a,b,c} 
	and spatial cues. %\cite{a,b,c}. 

Examples of \codenavigation cues are 
	recalling a name,
	contacting a person with expertise or 
	asking questions on a mailing list where people share their expertise, or 
	an episodic memory like having heard been presented a solution to their current problem at a conference years ago, and 
	sometimes even spatial cues like recalling that a piece of coude is located at the end of a text file. 

Tool support for following-up upon these cues, however, is typically limited to text file processing or hyperlinking of code elements at best. 

In our research, we are interested in how to support code navigation and code understanding with development tools. In interviews with professional software engineers, we looked at how developers find answers to technical questions in order to learn about their \codenavigation strategies \cite{Kuhn11xxx}. We found that the way towards an answer is typically split in two parts. In the (1) first part, developers aim to turn their fuzzy initial cue into a more concerete textual cue, for example by running a series of web searches and inspecting the results. Eventually, in the (2) second part they use that textual cue to query local documentation or the internet for the answer. We found that they particularly struggle with resolving social, episodic and spatial cues during the first part. The above behavior might be an artifact of current tool limitations, that is developers aim to find a lexical cue first because other cues are harder to follow-up, or it might be a general search behavior. In either case, developers do need better tool support to follow-up on social, episodic and spatial cues.

Of the four kinds of cues that software developers use, spatial cues stand out for not having a first-class representation in the ecosystem of source code. Textual cues typically relate to identifiers found in source code or to terms used in comments and documentation. Social cues typically relate to the personal network or to experts that share their knowledge on the internet. Episodic cues relate to past experience that is often recorded in version control system, mailing list archives or recorded conference talks. Spatial cues however typically lack such a first-class counterpart since neither software system nor their documentation do have an inherent spatial structure. 

Nevertheless, we found that developers do refer to \adhoc spatial properties, auch as the position of a piece of code on-screen or within a source text file. A mayor part of our research thus focused on creating and establishing a spatial representation of software systems that facilitates code navigation and understanding by spatial cues. A key contribution of our work is \emph{Software Cartography}, a novel cartographic visualization of software systems that is supposed to support code orientation by on-screen spatial cues \cite{Kuhn08a,Kuhn10b}. Code maps, as well call them, are a stable and spatial representation of software systems based on lexical information found in the system's source code. Code maps can be used by both individual developers as well as shared by a team. We evaluated our approach in a qualitative user study, with both professional and student users, and found that it is most helpful for exploring search results and call hierarchies \cite{Kuhn10c}.
...
In the context of this work, we are interested in supporting software engineers when they are facing technical questions that involve code navigation and code understanding. We introduce the term \emph{\codenavigation{}} in order to refer to those navigation and understanding activities that happen while developer are looking for an answer to their technical question. Code orientation is thus am umbrella term of code navigation and code understanding. 

In the following we aim to characterize \codenavigation according to the kind of cognitive cues being followed-up (lexical cues, social cues, episodic cues and spatial cues) and characterize \navigation tasks according to their aim (refind, discovery and learning) and their reach (ranging from current working set to the entire internet). 

\subsection*{Aim and Reach of Code Orientation}
	
The aim of code orientation tasks can be threefold. It can be about refinding a given piece of code or functionality that developers have encountered or heard of in the past. It can be about discovering a piece of code that you know must exist, by searching your local code repository or the internet for code examples. Or, it can be about learning how to implement a certain piece of code, by talking to people with expertise, by reading a book, or by finding advice on a mailing list or a blog. 

The reach of code orientations can ranges from the current working set to the entire internet. The current working set can be a single method or all open files. The local codebase can be the current project and the entire codebase of a developer's company, but also includes all local documentation and learning resources. Eventually the ultimate reach is accessing the internet, which offers two kind of codebases. On the one hand there are dedicated code repositories that only contain source code, on the other hand, there is an amazing amount of code examples that are contained in blog posts and websites. The latter are a most useful for code orientation since they are collaboratively pre-filtered, the authors of the embedding content carefully selected those examples for their usefulness for learning and copy-pasting. 

When going from a local to a global reach, the nature of information resources changes. Local information sources are typically limited, homogenous and authored by a small group of people with known expertise and known trustworthiness. More global information sources are typically unlimited, heterogenous and authored by people with unknown expertise and unknown trustworthiness.

Refinding tasks are typically targeted at the local codebase, with which the developers are familiar, whereas discovery and learning tasks are typically targeted at the internet. But just as likely, parts of the local code based might be unknown to a developers and thus he target of a discovery task locally. Or it may be that a developers recall that he read about the answer on the internet and thus he targets a refinding task globally.

In the following, we enumerate orientation cues by category. We motivate their use by developers, provide examples, shed light on underlying assumptions, and discuss current tool support and its shortcomings.

\subsection*{Orientation by Lexical Cues}

% Importance
Lexical cues are by far the most common cue used by software engineers for \codenavigation. 
% Examples
Examples of lexical cues are recalling the name of an identifier used to refind some code, or having a guess about a keyword used to search the web for documentation or code examples. 

% Assumption
Developers do rely on lexical cues because they assume that names are meaningful, that there is a meaning to names and that names have been meaningfully chosen. 
% Kind of information?
Lexical cues are typically pointers to lexical information that is present as identifier names or in comments. However, lexical information is not limited to source code but also found in the content of emails, bug reports and web pages. For example, following-up a lexical cue might guide the developer to a wikipedia page that contains the algorithm he's looking for. 

% Tool support and hint a need that we address later on?
Simple keyword search and regular expressions are of great help to follow-up on a lexical cue. However, a major problem with lexical cues is that developers to have make a guess about how other people name things. Using information retrieval and natural language processing can help to address this limitation. 

\subsection*{Orientation by Social Cues}

% Importance
Social cues are often not considered part of software comprehension, yet they are most powerful for discovery and learning. 
% Examples
Examples of social cues can be asking another person for help, or posting a question to a discussion board on the web where experts gather and share their expertise. 

% Assumption 
Developers do rely on social cues because they assume that people do have expertise, in particular that other people do have more or different expertise so they can be of help to find answers. 
% Kind of information?
Social cues are typically pointers to other persons from the developer's personal network, either a co-worker or a friend. However, social cues are not limited to the personal network but can also be pointers to mailing lists and other expert groups that are ready to share their expertise online on the internet.

% Tool support and hint a need that we address later on?
Support for code orientation by social cues is typically not present in development tools, thus the current state of the art is that developer have to recall the name of a person with expertise. This basically boils down to a lexical cue that has to be used a proxy for the actual social cue that developer does want to follow-up. Using techniques and ideas drawn from social media can help to address this limitation.

\subsection*{Orientation by Episodic Cues}

% Importance
Episodic cues are most helpful to refind source code that has been written or used in the past, since as humans we have strong episodic memories. 
% Examples
Episodic cues are typically tied to personal memory, as for example in recalling the first-hand experience of a conference talk or of a pair programming session. 

% Assumption 
Developers do rely on episodic cues because they assume that their knowledge in the past has been more accurate than their current knowledge. Which is a valid assumption because as humans our episodic memory works way better than our structural memory, typically developers do recall ``that'' they knew the answer before but not ``what'' the answer exactly constituted of.  

% Kind of information?
Episodic cues are typically pointers to past interaction with books, mailing lists and other people, but may also be pointers to past snapshots of the current or a related software system. 
% Tool support and hint a need that we address later on?
Support for code orientation by episodic cues in development tools is typically not present. Information that could be used to resolve an episodic cue is stored in external databases, such as version control repositories and mailing list archives, and thus often not integrated with development tools. In addition, the information stored those data silos is typically to low-level to serve as search space for episodic memories. Using data mining techniques and embedding the results as story-telling visualizations in the IDE can help to address this limitation.
...
\subsection*{Orientation by Spatial Cues}

% Importance
Spatial cues may help to reduce the cognitive load of a user's orientation in a hyperlinked document space, such as software systems. 
% Examples
% Kind of information?
Spatial cues can be of three kinds, either they are structural or conceptual as in recalling which class or architectural layer some functionality belongs to, or they are true spatial cues as in recalling the on-screen or within-text-file position of a given function.

% Assumption 
Developers do rely on spatial cues because as humans they have strong spatial capabilities. The spatial capabilities of our brain are impressive: it has been found that developers do form a spatially meaningful internal mental model of software systems even though the external representation of source code has no inherent spatial properties by itself. 

% Tool support and hint a need that we address later on?
Current support for code orientation by spatial cues in development tools is \adhoc at best. The systems is presented as a tree view of alphabetically ordered files and inside a file the source is linearized as an unordered text file. While this might accidentally help to refind some code by a spatial cue, support for the discovery of yet unknown source code by spatial cues is limited to structural cues. Providing developers with a cartographic on-screen visualization, so they can use spatial cues, may help to establish a novel kind of code orientation.
...
\begin{quote}
To support code navigation and code understanding, we need development tools that provide first-class support for the cognitive cues that developers use. In particular, we need better tool support for social cues and episodic cues. And third, since software has no inherent spatial dimensions, we need to establish a stable cartographic representation of software systems based on other properties in order to facilitate code orientation by on-screen spatial cues.
\end{quote}
...
In this work, we present the following contributions that explore the support of code orientation cues by development tools. Each of these contributions has been published as one or more peer-reviewed international conference paper or as a peer-reviewed international journal paper.  

Common to all these contributions is that they tap on unconventional information found in source code, in order to provide developers with new means of following-up code orientation cues, that would have been out of their reach without the novel tool support. For example, in \autoref{the chapter on chronia} we present a story-relling visualization of a system's history that enables new hires to draw on episodic memories that they would not have access to otherwise.
...
Current tool support for code orientation by spatial cues is \adhoc at best, most striking is the lack of spatial on-screen representations of source code. Without such a representation, developer are barely able to draw on the strong spatial capability of the human brain. 

We provide a cartographic on-screen visualization so they can start using spatial cues for code orientation. Since software has no inherent spatial structure, we use lexical and structural information found in the source code to establish a spatial layout of the local code base. Code maps, as we call them, are stable over time and can be shared among members of a team to establish a common mental model of the system. 
%
...
Keyword matching and regular expressions are powerful means for code orientation by lexical cues. However, current tool support fails to meet the developer needs when they are following-up on a fuzzy lexical cues. 
...
For more in information please refer to the \autoref{the chapter on lexical cues}. 
%
...
Current tool support for code orientation by spatial cues is \adhoc at best, most striking is the lack of spatial on-screen representations of source code. Without such a representation, developer are barely able to draw on the strong spatial capability of the human brain. With \SOCA we aim to address this limitation. We provide a cartographic on-screen visualization so that software engineers can start using spatial cues for code orientation. Since software has no inherent spatial structure, we use lexical and structural information found in the source code to establish a spatial layout. 

\SOCA embeds a cartographic visualization of the current working set in the development environment (IDE) of software engineers. Code maps, as we call them, are stable over time and can be shared among members of a team to establish a common mental model of the system. Code maps are most useful when they supports as many development tasks as possible with spatial cues. Therefore we integrated \SOCA in the IDE so that a map of the software system may always be present and may thus support as many development tasks as possible. 
...
\begin{description}
% --> related.tex
\item[\autoref{the chapter on related work}] discusses related work of this thesis. We present various user studies and solutions to code orientation and analyse the short comings in the context of each orientation cue.
% --> codemap.tex
...
\item[\autoref{the chapter on the MSR user study}] presents a user study that looked at how developers find answers to technical questions and discusses code orientation by cognitive cues.
% --> gutter+stars.tex
\item[\autoref{the chapter on lexical cues}] presents an approach to cluster and summarize software systems using lexical information found in source code. The approach is implemented in the \textsc{Hapax} tool. 
...
%- Photographic memory.
%- The cues have to be turned into actions.
%- How many (inter)actions do you have to do to follow up one cue.
...
%- Erwann: information can come right from the source code (lexical cues) and can come from somebody else (social cues) from temporal memory or version control system (temporal ones) ...
%- Cues are not about the sources of information, but about the way developers navigate about how they cognitively work when they follow.
...
%- Codemap: if you observe which cues devs use to refer to code, you'll find all four, but if you look at tool support, you find lexical supported by tools, and temporal only in external tools, and social cues are also not supported, you gonna recall the name of person (ie using the lexical cue of the name as a proxy for the social connection) and spatial is not supported at all, we got the IDE layouted without care about spatial thinking ... code bubbles, code map, and code canvas are about the first systems to take care of this ... 
...
%- So this why codemap is so interesting because it takes the lexical information and give it a spatial structure in the hope to fill the gap when devs are stuck with lexical cues or social/temporal
%- Spatial cues "somewhere at the end of this file"
...
%- Erwann: if the notion of mental model can be used for all cues ... the other tools gravitate around this spatial mental model.
	
%- All these tools help you to follow up, some more complex cues and unconventional information. It is not that any of these tools addresses a very specific task? Erwann thinks yes it does.
%- I want to help developers, working with code, in particular wrt navigation and understanding, and learning about code.  These cues can be  (ii)  or (iii) it could be temporal or episodic, having a memory from the past that could lead you to an answer, that you've seen it last year (or recall the song that played on the radio) or (iv) 
%- maybe in some years we can add here location based cues, like I've written that code on the train. And in the source of JUnit Kent Beck found it so cool that he added "was written on HŸtte at 1200 mUm"
%- and sometime people follow up some very underdeveloped spatial cues. So we know they follow up these cues so we need better tool support and not have to be force to use lexical proxies, you should not be forced to recall the name, but be able to directly work social information when following up a social cues.
